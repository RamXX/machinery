...
|---|---|---|---|---|
| `saveUser` | actor | `(input{userId,status,actor}) -> UserRow \| err{ErrConstraint,ErrConflict,ErrDiskFull,ErrTimeout,ErrLocked}` | pre: guard passed, tx open. post: row `status` atomically, or unchanged on err CARRIES{column:User.status} | C4 `crm.domain -> crm.repo` then `crm.repo -> store` (SurrealQL SaveUser) |
