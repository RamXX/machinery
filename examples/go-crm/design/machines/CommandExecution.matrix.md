...
|---|---|---|---|---|
| `openDatabase` | actor | `(input{dbPath}) -> Tx \| err{ErrLocked,ErrCorrupt,ErrUnavailable}` | post: DB open for writing, or typed error CARRIES{sink:store} | C4 `crm.commands -> crm.repo` then `crm.repo -> store` (Repo.Open) |
