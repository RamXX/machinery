...
\* Generated from Shipment.machine.json by tools/tla_gen.py. Control-flow model.
\*
...
\*      - rolledBack: priorStatus is set on every path into the overlay from a domain state; only Pending, Dispatched, and InTransit reach the overlay (Delivered and Lost are final), and all three priorIs* guards are present
