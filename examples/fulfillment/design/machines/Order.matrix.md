...
Placement (ARCHITECTURE.md 5): a status row in the Order DB, optimistic lock, mutations persist in one
...
| `validateLineItemQuantity` | invariant | `(quantity) -> bool` | accept an item only when `quantity > 0` | inv `line-item-quantity-positive`; LineItem add validation | property | generated integer quantities at the Order Service API boundary |
| `orderDeliveredTerminal` | invariant | `(state) -> bool` | Delivered, Cancelled, and Failed are final and expose no outgoing transitions | inv `order-delivered-terminal`; Order lifecycle structure | model + unit | machine graph final-state assertion |
...
| Corrupt or absent rollback route | no `priorIs*` guard admits after persistence failure | `rolledBack -> routingFault` | stop the machine and page for data repair; never guess a domain status | explicit terminal residual; `recordRoutingError` preserves the diagnosis |
