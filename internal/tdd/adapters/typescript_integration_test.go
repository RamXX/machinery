...
func tsAssetBytes(t *testing.T, rel string) []byte {
	t.Helper()
	body, err := os.ReadFile(filepath.Join(repoRoot(t), "internal", "tdd", "adapters", "assets", "typescript", filepath.FromSlash(rel)))
	if err != nil {
		t.Fatal(err)
	}
	return body
...
func laneSeedRoot(t *testing.T, includeFragment bool, tamperConformance func(source string) string) string {
	t.Helper()
	root := t.TempDir()
...
// TestContributorLaneRejectsRuntimeAbsence proves a stale node runtime fails
