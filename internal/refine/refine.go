...
func emitLifecycleImpl(machine, sem *ir.Value, sourceNames [2]string) (string, map[string]string) {
	...
	header := fmt.Sprintf(`\* GENERATED by tools/refine_gen.py from %s + %s.
...
func ReconcileTerminal(machine, sem *ir.Value) (phases []string, success string, failures []string, retries []retrySpec, err error) {
	defer func() {
		if r := recover(); r != nil {
			if ee, ok := r.(*ExitError); ok {
				err = ee
	...
	return phases, success, failures, retries, nil
}
...
func EmitTerminal(machine, sem *ir.Value, sourceNames [2]string) (string, map[string]string, error) {
	so := sem.AsObject()
	mid := ir.Title(so.GetString("machine"))
	phases, success, failures, retries, err := ReconcileTerminal(machine, sem)
	if err != nil {
		return "", nil, err
	}
	maxr := intNum(so.Get2("max_retries"))
	...
	header := fmt.Sprintf(`\* GENERATED by tools/refine_gen.py (terminal-lifecycle) from %s + %s.
...
\* on the success path, MaxRetries = %d, and single-instance execution.`, sourceNames[0], sourceNames[1], maxr)
	...
	for i, p := range phases {
		...
		L = append(L, fmt.Sprintf(`Fail_%s == st = "%s" /\ st' = "%s" /\ %s`, p, p, failTo, unch(nil)))
...
func EmitSaga(machine, sem *ir.Value, sourceNames [2]string) (string, map[string]string, error) {
	...
	var L []string
	L = append(L, fmt.Sprintf("---- MODULE %sData ----", mid))
	L = append(L, fmt.Sprintf(`\* GENERATED by tools/refine_gen.py (saga pattern) from %s + %s.`, sourceNames[0], sourceNames[1]))
...
func Run(machinePath, semPath, outdir string) error {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "refine_gen: "+err.Error())
		os.Exit(1)
	}
	...
	default:
		fmt.Fprintf(os.Stderr, "refine_gen: unsupported pattern %s (linear-lifecycle, terminal-lifecycle, saga)\n", ir.Repr(pat))
		os.Exit(1)
	}
	if genErr != nil {
