...
func stop(w io.Writer, root string, cfg Config, in Input, warn string) error {
	...
	shouldBlock := drift > 0 || g4Blocking > 0 || (cfg.Strict && blocking > 0)
	switch {
	...
	case blocking > 0:
		...
		clearState(root, in.SessionID)
		return emitJSON(w, stopOut{SystemMessage: fmt.Sprintf(
			"machinery: %d gate ERROR finding(s) remain (no DRIFT); normal mid-phase. "+
...
func sessionStart(w io.Writer, root string, cfg Config, warn string) error {
	...
	if cfg.Impl != "" {
		fmt.Fprintf(&b, "- Import-boundary gate G4 watches source edits under %s (against the Architecture Contract).\n", cfg.Impl)
	}
	fmt.Fprintf(&b, "- Generated artifacts are read-only and hooks deny edits to them: %s/**/*.oracle.md, %s/formal/*.tla and *.cfg, %s/packs/**, %s/pack/**. Edit the sources, then regenerate (machinery oracle | machinery verify-formal --gen-only | machinery pack generate).\n",
