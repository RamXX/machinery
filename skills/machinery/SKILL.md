...
The deterministic gates live in `machinery check <design> [--impl <dir>] [--gate gm,gs,gp,gi,gn,g2,g3,gx,g4,g5]`
...
`formal/{policy,integrity,isolation}.relational.yaml` exists; see Recursive decomposition, the
...
- **Phase 2** records the existing architecture and declares the INTENDED boundaries. Then run
  ...
  because Gx has no per-entity waiver.
...
- `machinery check` - the deterministic gate suite (Gm-transition, Gs-surface, Gp/Gi/Gn
  relational gates, G2-c4, G3-machine, Gx-trace, G4-import,
