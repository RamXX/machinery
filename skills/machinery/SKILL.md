...
The deterministic gates live in `machinery check <design> [--impl <dir>] [--gate gm,gp,gi,gn,g2,g3,gx,g4,g5]`
(g5 runs automatically on decomposed designs; gm runs once `migration.yaml` exists; gp, gi, and gn each run automatically once the matching
`formal/{policy,integrity,isolation}.relational.yaml` exists; see Recursive decomposition and the
Phase 1 relational layers).
...
relational model is an enforcement artifact). Once code
...
- `machinery check` - the deterministic gate suite (Gm-transition, Gp/Gi/Gn relational gates,
  G2-c4, G3-machine, Gx-trace, G4-import,
