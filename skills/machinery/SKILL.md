...
The deterministic gates live in `machinery check <design> [--impl <dir>] [--gate gp,gi,gn,g2,g3,gx,g4,g5]`
(g5 runs automatically on decomposed designs; gp, gi, and gn each run automatically once the matching
...
under all constraints), per-unique `Distinct` witnesses. Value domains are bounded to the exact type
