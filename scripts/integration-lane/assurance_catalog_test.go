...
func assuranceSeed(t *testing.T, root string) {
	...
	// MAC-avfp compatible extension (mirroring the approved MAC-wi2u seed
	// extension): downstream adapter fragments declare native-conformance
	// sources inside the adapters' exclusively owned asset directories
	// OUTSIDE testdata/integration-lanes. Seed every declared fragment source
	// from the repository so the closed catalog validates identically in
	// fixture roots; frozen v1 and probe seeding semantics are unchanged and
	// the copy is idempotent for sources already seeded above.
...
func TestAssuranceCatalogExecutesFourLanguageProbesNatively(t *testing.T) {
	...
	// MAC-avfp RED supersession (justified, approved with this RED, mirroring
	// the approved MAC-wi2u supersession): the exact count 4 pinned the
	// probe-only catalog state. The closed catalog contract
	// (assurance.CONTRACT.md) declares the downstream adapter stories
	// (MAC-wi2u Go, MAC-avfp TypeScript, MAC-imtz Python, MAC-8yai Elixir)
	// each add a NEW named native-conformance fragment, so the union count is
	// >= 4 with all four frozen probe suites still present and exactly
	// accounted below. The four probe receipts and the per-adapter probe
	// accounting stay byte-exact.
