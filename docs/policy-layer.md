...
The policy layer is one of three sibling relational algebras. Its siblings are the
[integrity layer](integrity-layer.md) (structural invariants: uniqueness, singletons, cardinality)
and the [isolation layer](isolation-layer.md) (multi-tenant reference isolation). Each is
independently opt-in and generated by the same `machinery alloy` command.
