...
| `events` | `event(id)`, `event_edge(edge, event, producer, consumer)`, `event_edge_payload_field(edge, field)`, `event_producer(id, producer)`, `event_participant(id, participant)`, `event_consumer(id, consumer)`, `event_payload_field(id, field)` | `event:name`, `edge:<event>\|<producer>-><consumer>` | ARCHITECTURE.md event-contract tables, one row per producer-consumer edge: `event` is defined once, at the first row naming it; each row yields one `event_edge` per pair of its producer and consumer cells (the cross product), with a content-derived id and the row's closed payload set as `event_edge_payload_field`; `event_payload_field` is the union over the event's edges; one edge restated with one payload collapses, and with two payloads fails the projection naming both rows |
| `c4` | `c4_element(id, kind, parent)`, `c4_relationship(src, dst)`, `boundary(id, element, role)`, `allowed_edge(src, dst)`, `reads_row(artifact, reader)`, `no_machine_waiver(component)`, `action_owner(action, component)` | `c4:id`, `boundary:id`, `external:id`, `placement:Name`, `action:Entity.name` | `workspace.dsl` and the Architecture Contract; `no_machine_waiver` for each persistence-and-placement row whose first component carries `(no machine: <reason>)` with a reason (a contract-only record when that component has a matrix and no machine); `action_owner` for each `Entity.action` of an ARCHITECTURE.md action-ownership table row and each backticked component its owner cell names (an `(unowned: <reason>)` row projects nothing) |
...
- **Duplicates fail.** Two rows of a defining relation (see the schema description) that give one
  ...
  Identical tuples collapse: a relation is a set. Two exceptions are by design: an event spanning
  several event-contract rows is one `event` and several `event_edge` rows, and two relationships
  between one entity pair with one cardinality and no role or name are one `relationship` tuple
  (Gy-rules asks the model to name them; the 1.0 `model` block still refuses them).
...
Input `source` paths are portable registry-relative paths: each one resolves against the directory
...
## Containerized CI: the daemon must see the bind sources

`verify-checkers` keeps checker scratch in a private OS temp directory and hands the engine exactly
two binds from it: the writable `/work` and the read-only `/checker` inputs mount. The OCI engine is
machinery's local control plane, resolved from `runtime.engine` and required to be a regular
non-symlink executable that is copied and content-rechecked before use; machinery never assumes an
ambient engine is already available, and a missing engine or unreachable daemon is an ERROR before
anything runs.
...
config for type "bind": bind source path does not exist` (or a permission error on the source) while
...
| bind source path invisible to the engine daemon | ERROR from the daemon at container create (`invalid mount config ... bind source path does not exist`) |
