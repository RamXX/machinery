...
| `matrices` | `unit(id, machine, name, kind)`, `unit_clauses(unit, clause, status)`, `unit_reads(unit, event, field)`, `unit_values(unit, group, member)`, `unit_derived(unit, fact)`, `unit_payload(unit, event, field)`, `unit_writes(unit, fact)`, `unit_uses(unit, fact)`, `unit_carries(unit, kind, target)`, `unit_produces(unit, action)`, `unit_declares(unit, group)` | `unit:M.name`; `matrix:M` for a declaration on a non-unit row (a consumed-event row) | `machines/*.matrix.md` declaration groups: `CLAUSES{}`/`RETIRED{}`, `READS{}`, `VALUES{}`, `derived:`, `payload {}`, `WRITES{}`, `USES{}`, `PRODUCES{}`, `CARRIES{}`; `unit_declares` marks each group present on the row (`WRITES`, `USES`, `PRODUCES`, `CARRIES`, `VALUES`, `CLAUSES`, `READS`, `payload`), so `WRITES{}` (read-only) differs from no WRITES group |
