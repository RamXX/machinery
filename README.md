...
- **machinery** -- the deterministic gate tools and formal generators, plus the agent skill and role
  ...
  **One line, no clone** (downloads the checksum-verified binary, then installs the skill; no Git, no
  ...
  `MACHINERY_VERSION=v0.7.2`, `INSTALL_DIR=/usr/local/bin`, `MACHINERY_HOMES="$HOME/Agent Home"`, or
  ...
  **Windows support:** v0.7.2 publishes a cross-compiled `machinery-windows-amd64` binary and
...
machinery update --version v0.7.2          # force an exact release
...
- `docs/` the deep dives: the [rebuild and hybrid guide](docs/rebuild-guide.md) (dual domain truths,
  ...
  citation kinds, the byte budget and its documented divisor, and the `gw` gate's coverage rule),
