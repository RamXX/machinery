...
func preflightRun() {
	...
	fmt.Fprintln(out, "  auto     'machinery verify-formal' downloads the TLA+ tools (tla2tools.jar) and, for designs with a policy annotation, the Alloy analyzer (org.alloytools.alloy.dist.jar) on first use, pinned and checksum-verified (that step needs Java)")
