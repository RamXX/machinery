...
and a synthetic design under both engines and requires identical outputs; `make dagger-job
JOB=datalog-parity` runs it against a pinned Soufflé.
...
| `payload.dl` | `payload_twin` | `payload {}` on matrix event rows against the Architecture Contract event row's closed payload set |
